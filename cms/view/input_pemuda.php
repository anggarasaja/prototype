...
<script type="text/javascript">
    ...
    //var caborOutput;
          var table;
        function pageLoad(){
                        $('#datatable').html( '<table cellpadding="10" cellspacing="5" border="0" class="table table-striped table-hover" width="100%" id="dataTablesPemuda"><\/table>' );
                        $.ajax({url: '<?php echo $url_rewrite.'core/input_pemuda/read_all_pemuda.php'; ?>',
                                type: 'POST',
                data: {id_propinsi:0},
                success: function(output) {
                        var op=JSON.parse(output);
                                        table = $('#dataTablesPemuda').DataTable( {
                                "order": [[ 0, "desc" ]],
                                "language": {
          ...
          map = new google.maps.Map(document.getElementById('gmap_city'),
               ...
               /* $.ajax({url: "<?php echo $url_rewrite.'core/input_pemuda/read_cabor.php'; ?>",
             success: function(output) {
                //alert(output);
                caborOutput = output;
       ...
       $(document).on('click', '.details-control', function () {
            ...
            // Open this row
                 ...
                 $('#jenkel-update-'+id_row +'-perempuan').prop("checked", true);
						...
						$("#keterangan-update-"+id_row +"").val(keterangan_row);
						                 
                 
        				$('#update-pemuda-'+id_row +'').submit(function(event) {
          ...
          //alert($('#lng-update-'+row.data()[0] +'').val() );
                     var posting = $.post(url, {
		                 id_pemuda: id_row, 
		                 namapemuda: $('#nama-pemuda-update-'+id_row +'').val().trim(), 
      ...
      /* Alerts the results */
	                    ...
	                    row.data( 
	                      [
	                              id_row,
	                              $('#nama-pemuda-update-'+id_row +'').val().trim(),
                ...
                error: function (xhr, ajaxOptions, thrownError) {
                        alert(xhr.status + " "+ thrownError);
                                ...
                                //var caborValue = $(this).val();
                                   ...
                                   $.ajax({url: '<?php echo $url_rewrite.'core/input_pemuda/delete_pemuda.php'; ?>',
                                type: 'POST',
                                data: {id_pemuda:id_row},
                                success: function(output) {
                                  //alert(output);
...
<div id="inputpemuda" class="input-box">
<form id="input-pemuda" action="<?php echo $url_rewrite.'core/input_pemuda/input_pemuda_controller.php'; ?>" method="post">
<div class="row">
...
<p>Anda yakin akan menghapus data pemuda ini ?</p>
</div>
...
$('#lat').keyup(function () {
                ...
                $("#input-pemuda").submit(function(event) {
      ...
      /* get some values from elements on the page: */
        var $form = $( this );
          var url = $form.attr( 'action' );
          if ($('#nama-pemuda').val()=="" || $('#city_list').val()==0 || $('#jenkel:checked').val()==undefined ) {
                if($('#nama-pemuda').val()==""){
                        $('#alert-input-failed').append('<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert">&times;<\/a>Silakan isi nama pemuda.<\/div>');
    ...
    var tableString = '<form id="update-pemuda-'+d[0] +'" action="<?php echo $url_rewrite.'core/input_pemuda/update_pemuda.php'; ?>" method="POST">'+
    '<table cellpadding="5" cellspacing="0" border="0"  class="table" style="padding-left:50px;border: 0;">'+
        '<tr>'+
                        '<td style="padding : 10px">ID Pemuda<\/td><td style="padding:10px 20px 10px 20px;">:<\/td>'+
            '<td><input tabindex="1"  style="width:230px; height:34px;" type="text" name="id-pemuda-update-'+d[0] +'" id="id-pemuda-update-'+d[0] +'" class="form-control" value="'+d[0]+'" readonly><\/td>'+
        ...
        '<\/tr>'+
        '<tr>'+
             '<td style="padding : 10px">Nama Pemuda<\/td><td style="padding:10px 20px 10px 20px;">:<\/td>'+
            '<td><input tabindex="1"  style="width:230px; height:34px;" type="text" name="nama-pemuda-update-'+d[0] +'" id="nama-pemuda-update-'+d[0] +'" class="form-control" value="'+d[1]+'"><\/td>'+
